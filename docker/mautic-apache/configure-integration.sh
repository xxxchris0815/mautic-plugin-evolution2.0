#!/usr/bin/env bash
# Publish Evolution integration against the local mock API.
set -euo pipefail

BASE_URL="${BASE_URL:-http://localhost:8080}"
ADMIN_USER="${ADMIN_USER:-admin}"
ADMIN_PASS="${ADMIN_PASS:-MauticAdmin123!}"
API_URL="${EVOLUTION_API_URL:-http://host.docker.internal:8081}"
API_KEY="${EVOLUTION_API_KEY:-test-evolution-key}"
INSTANCE="${EVOLUTION_INSTANCE:-cloud-instance}"
...
data=urllib.parse.urlencode({"_username":USER,"_password":PASS,"_csrf_token":csrf}).encode()
...
token=re.search(r'id="integration_details__token"[\s\S]*?value="([^"]+)"', form).group(1)
payload={
...
req=urllib.request.Request(BASE+"/s/plugins/config/MauticEvolution", data=urllib.parse.urlencode(payload).encode(), method="POST")
req.add_header("Content-Type","application/x-www-form-urlencoded")
req.add_header("Referer", BASE+"/s/plugins/config/MauticEvolution")
resp=opener.open(req)
body=resp.read().decode()
print("save:", "enabled" if '"enabled":1' in body or '"enabled": 1' in body else body[:300])
